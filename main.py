...
# Codex's native image tool writes files into the shared output directory and
# may reuse a descriptive filename. Keep capture + archival atomic so concurrent
# canvas tasks cannot read or overwrite one another's result.
CODEX_IMAGE_OUTPUT_LOCK = asyncio.Lock()
...
async def generate_codex_provider_image(prompt, size, model, reference_images=None, provider=None, output_key=""):
    ...
    try:
        ...
        # helper here made an otherwise configured node fail before Codex's
        ...
        async with CODEX_IMAGE_OUTPUT_LOCK:
            ...
            raw = await run_codex_cli(
                image_prompt,
                # `$imagegen` is an in-prompt native tool instruction, not a
                # Codex CLI `--model` value. Keep the user's CLI default model.
                model="",
...
async def codex_status():
    ...
    try:
        ...
        return {
            ...
            "path": exe,
            "message": "GPT CLI 已安装。画布生图会使用当前 Codex CLI 登录态与画布配置；登录状态会在首次执行时由 CLI 校验。" if ok else (err_text or out_text or "Codex CLI 检测失败"),
...
# ============================================================
# Canvas Agent integration: implementation lives in agent/backend.py.
# Keep this explicit dependency list as the only bridge to upstream services.
# ============================================================

from agent.backend import (
    canvas_task_payload_dict as _canvas_task_payload_dict,
    configure_agent_backend,
    persist_canvas_generation_task as _canvas_generation_task_persist,
    router as codex_agent_router,
    startup_agent_backend,
)

configure_agent_backend(
    codex_cli_resolver=codex_cli_executable,
    AIReference=AIReference,
    API_ENV_FILE=API_ENV_FILE,
    ASSETS_DIR=ASSETS_DIR,
    BASE_DIR=BASE_DIR,
    CANVAS_AGENT_IMAGE_SIZE_OPTIONS=CANVAS_AGENT_IMAGE_SIZE_OPTIONS,
    CANVAS_TASKS=CANVAS_TASKS,
    CANVAS_TASK_LOCK=CANVAS_TASK_LOCK,
    CANVAS_TASK_WORKERS=CANVAS_TASK_WORKERS,
    CHAT_RATIO_SIZE_OPTIONS=CHAT_RATIO_SIZE_OPTIONS,
    CODEX_DEFAULT_IMAGE_MODELS=CODEX_DEFAULT_IMAGE_MODELS,
    CanvasVideoRequest=CanvasVideoRequest,
    GEMINI_CLI_DEFAULT_IMAGE_MODELS=GEMINI_CLI_DEFAULT_IMAGE_MODELS,
    JimengPendingError=JimengPendingError,
    OUTPUT_DIR=OUTPUT_DIR,
    OUTPUT_INPUT_DIR=OUTPUT_INPUT_DIR,
    OUTPUT_OUTPUT_DIR=OUTPUT_OUTPUT_DIR,
    OnlineImageRequest=OnlineImageRequest,
    create_canvas_image_task=create_canvas_image_task,
    create_canvas_video_task=create_canvas_video_task,
    jimeng_query_result=jimeng_query_result,
    jimeng_store_outputs=jimeng_store_outputs,
...
app.include_router(codex_agent_router)
app.on_event("startup")(startup_agent_backend)


# ============================================================
# Canvas Agent integration ends here.
# ============================================================
